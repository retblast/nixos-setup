let
  fontsOverlay = (
    final: prev: {
      ...
      # menu = 9
      # css = "ultra-expanded"
      iosevka-retblast = prev.iosevka.override {
        privateBuildPlan = {
          family = "retblast's Iosevka setup";
          spacing = "normal";
          serifs = "sans";
          noCvSs = false;
          exportGlyphNames = true;
      ...
      input-fonts = prev.input-fonts.overrideAttrs (old: {
        ...
        src = prev.fetchzip {
          ...
          hash = "sha256-iVm/KQRKYrJs37NHZQchVugY63B7egdduYfi7dmvE/4=";
          stripRoot = false;
        };
      });
      ...
      commit-mono-retblast-script = prev.writers.writeFishBin "cmsc" ''
        			set -l options 'srcPath=' 'localPath=' 'inputFontFormat=' 'outputFontFormat='
        			...
        			for font in "$_flag_srcPath"/*."$_flag_inputFontFormat"
        				...
        				$command -f 'ss03,ss04,ss05,cv02,cv06,cv10' -S -U Retblast -R 'CommitMonoV143/CommitMono' $font "$_flag_localPath"/(string replace .$_flag_inputFontFormat "" (string replace "$_flag_srcPath" "" $font))-retblast."$_flag_outputFontFormat"
      ...
      commit-mono-retblast = prev.callPackage ./localDerivations/commit-mono-retblast.nix { };

      # prev.commit-mono.overrideAttrs(old: {
      #   postInstall = ''
      #     ${prev.commit-mono-retblast-script}/bin/cmsc
      #   '';
      # });
      ...
      jetbrains-mono-otf = prev.callPackage ./localDerivations/jetbrains-mono-otf.nix {
        fontmake = prev.python3Packages.fontmake;
      };
      noto-fonts-otf = prev.callPackage ./localDerivations/noto-fonts-otf.nix { };
      # Actually OTF, but do this because the package name has ttf on it argh
      # liberation_ttf = prev.callPackage ./localDerivations/liberation-otf.nix {
      #  fonttools = prev.python3Packages.fonttools;
      #   fontforge = prev.fontforge;
      # };
      roboto-otf = prev.callPackage ./localDerivations/roboto-otf.nix {
        fonttools = prev.python3Packages.fonttools;
        fontmake = prev.python3Packages.fontmake;
