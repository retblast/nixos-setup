let kernelsOverlay = (
	final: prev: {
		# XanMod Kernel
		linux-xanmod-retblast = prev.linuxPackages_xanmod_latest.extend (kfinal: kprev: {
		...
		linux-retblast-zen = prev.linuxPackages_zen.extend (kfinal: kprev: {
			kernel = kprev.kernel.override {
				argsOverride = {
					structuredExtraConfig = with prev.lib.kernel;
						# Merge the zen kernel's own config with this one using the // operator
		...
		# Default
		linux-retblast = prev.linuxPackages_latest.extend (kfinal: kprev: {
			kernel = kprev.kernel.override {
				argsOverride = {
					structuredExtraConfig = with prev.lib.kernel;
