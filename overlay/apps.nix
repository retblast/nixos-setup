let
  appsOverlay = (
    final: prev: {
      handbrake-retblast = prev.handbrake.override {
        useFdk = true;
      };
      ...
      # gimp-retblast = prev.gimp.override {
      # 	withPython = true;
      # };
      ...
      my-vscode = prev.vscode-with-extensions.override {
        vscodeExtensions = [
          ...
          # https://github.com/nix-community/nix-vscode-extensions/issues/127
      ...
      octaveFull = prev.octaveFull.withPackages (
        prev: with prev; [
          symbolic
        ]
      );
      ...
      #});
      koboldcpp = prev.callPackage ./localDerivations/koboldcpp.nix { };
    }
  );
in
appsOverlay
