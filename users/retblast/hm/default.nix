{ inputs, taihouConfig, config, lib, pkgs, ... }:
{
	...
	home = {
		username = "retblast";
		homeDirectory = lib.mkDefault "/home/retblast";
	};
		...
		services = {
			ollama-intel-gpu = {
				...
				Service = {
					...
					ExecStart = "/run/current-system/sw/bin/podman run --rm -p 127.0.0.1:11434:11434 -v /home/retblast/models:/mnt -v ollama-volume:/root/.ollama -e OLLAMA_KV_CACHE_TYPE='q8_0' -e OLLAMA_NUM_PARALLEL=1 -e OLLAMA_MAX_LOADED_MODELS=1 -e OLLAMA_FLASH_ATTENTION=1 -e OLLAMA_NUM_GPU=999 -e DEVICE=iGPU --device /dev/dri --name=ollama-intel-gpu localhost/ollama-intel-gpu:latest";
